...
public:
  ...
  GroundInitializer(const float lidar_height = 1.9f);
  ~GroundInitializer(){}

  void setGroundPlanefitParams(const Range2D& range = Range2D(-20.f,20.f,-10.f,10.f));
  bool groundInitializer(const PointCloudConstPtr in_cloud_ptr, Eigen::VectorXf& plane_model, float &estimate_lidar_height);
